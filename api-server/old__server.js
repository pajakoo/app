...
const CLIENT_URL = "http://localhost:3000"; //"https://deluxe-tapioca-7f56a1.netlify.app";//
...
passport.use(
  new GoogleStrategy(
    ...
    async function (accessToken, refreshToken, profile, cb) {
      ...
      // await client.connect();
      // const existingUserCursor = await db.collection('users').find({ _id: profile.id});
      // console.log(profile.id, 'gg:',profile, '+++++',  Object.keys(existingUserCursor),'----');
      ...
      await client.connect();
        try {
          ...
          const existingUserCursor = await db.collection('users').find({ googleId: profile.id});
          const existingUser = await existingUserCursor.next();
        console.log(!existingUser ,'pajak');
          if (!existingUser) {
...
passport.serializeUser(async (user, callback) => {
	...
	try {
		// Check if the user already exists in the database
		const existingUserCursor = await db.collection('users').find({ googleId: user.id });
		const existingUser = await existingUserCursor.next();
		console.log(!existingUser, 'pajak');
		if (!existingUser) {
			// If the user doesn't exist, create a new user
...
passport.deserializeUser(async (id, callback) => {
	await client.connect();
	try {
	  const user = await db.collection('users').findOne({ googleId: id });
	  callback(null, user);
	} catch (err) {
	  callback(err);
	} finally {
...
router.get("/login/success", async (req, res) => {
	await client.connect();
	const user = await db.collection('users').findOne({ googleId: '106392715545722847291' });
	console.log('pajak success:->', req.user, user);
	if (user) {
		res.status(200).json({
			error: false,
